...
export function parseBlogPost(html, sourcePath) {
  ...
  let title = h1Match ? h1Match[1].replace(/<[^>]*>/g, '').trim() : 'Infrastructure & Project Insights';
  title = decode(title);

  // Strip starting brand prefixes for clean, SEO-friendly H1 headers
  title = title
    .replace(/^AARAA Infrastructure Pvt\. Ltd\. (?:Becomes|Undertakes|Secures|Commences)?\s*/i, '')
    .replace(/^Aaraa Infrastructure Pvt\. Ltd\. (?:Becomes|Undertakes|Secures|Commences)?\s*/i, '')
    .replace(/^AARAA Infrastructure's\s*/i, '')
    .replace(/^Aaraa Infrastructure's\s*/i, '')
    .replace(/^AARAA Infrastructure\s+(?:Awarded|Secures|Undertakes|Commences|Strengthens)?\s*/i, '')
    .replace(/^Aaraa Infrastructure\s+(?:Awarded|Secures|Undertakes|Commences|Strengthens)?\s*/i, '')
    .replace(/^AARAA Infrastructure\s*/i, '')
    .replace(/^Aaraa Infrastructure\s*/i, '')
    .trim();
  if (title) {
  ...
  if (!heroImage || heroImage.includes('logo')) {
    heroImage = '/image/project-item/project-item-12.jpg';
  }
  ...
  // 8. Helper to parse Heading & Description for Gallery Cards
  function parseHeadingAndDescription(rawAlt = '', src = '', cat = '') {
    let alt = decode(rawAlt).trim();
    if (!alt || alt.toLowerCase() === 'gallery image' || alt.toLowerCase().startsWith('image')) {
    ...
    // Preposition split (e.g. "Site preparation and civil grading for 180 MWp Waaree project")
    const prepMatch = alt.match(/^(.*?)\s+(for|at|in|with|on)\s+(.*)$/i);
    if (prepMatch && prepMatch[1].trim().length >= 3) {
      const rawHeading = prepMatch[1].trim();
    ...
    if (words.length <= 4) {
      return {
        heading: alt,
        description: `${alt} during site execution for ${cat || 'AARAA Infrastructure project'}.`,
        alt
      };
    }

    const heading = words.slice(0, 4).map(w => (w.length > 2 ? w.charAt(0).toUpperCase() + w.slice(1) : w)).join(' ');
    return {
      heading,
      description: alt,
      alt
  ...
  // Extract Gallery Images (from articleHtml or whole html)
  ...
  while ((imgMatch = imgRegex.exec(searchHtmlForImages)) !== null) {
    ...
    if (!src.includes('logo') && !src.includes('brand') && !src.includes('avatar') && !src.includes('turnstile') && !src.includes('blog-post-') && !src.includes('raje') && src !== heroImage) {
      const parsedMeta = parseHeadingAndDescription(imgMatch[2] || '', src, category);
      galleryImages.push({
        src,
        heading: parsedMeta.heading,
  ...
  if (videoSrc) {
    ...
    if (!galleryImages.some(item => item.src === cleanVideoSrc)) {
      galleryImages.push({
        ...
        description: 'Celebration Video Highlights',
        alt: 'Celebration Video Highlights'
      });
    }
  }
