...
export function parseBlogPost(html, sourcePath) {
  ...
  const date = dateMatch ? dateMatch[1].trim() : 'July 2026';
  ...
  const location = locationMatch ? locationMatch[1].trim() : 'Chennai, India';
  ...
  const videoSrc = videoMatch ? videoMatch[1] : null;
